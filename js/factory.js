...
const Factory = (() => {
  ...
  const builders = {
    ...
    irrigator(m){
      const g = new THREE.Group();
      const tank = box(0.9, 0.55, 0.9, [M.metalDark, M.metalDark, M.chestTop, M.metalDark, M.metal, M.metalDark]);
      tank.position.y = 0.28; g.add(tank);
      const water = box(0.7, 0.32, 0.7, new THREE.MeshLambertMaterial({ color: 0x3f9fc0, transparent: true, opacity: 0.75 }));
      water.position.y = 0.5; g.add(water);
      const nozzle = box(0.16, 0.1, 0.4, M.metal); nozzle.position.set(0.3, 0.62, 0.35); g.add(nozzle);
      const spray = box(0.3, 0.5, 0.3, new THREE.MeshBasicMaterial({ color: 0x9ad8e8, transparent: true, opacity: 0.25 }));
      spray.position.set(0.32, 0.95, 0.55); g.add(spray);
      const lamp = box(0.12, 0.12, 0.12, M.glowOff); lamp.position.set(-0.32, 0.72, -0.32); g.add(lamp);
      m.animParts = { lamp, spray };
      return g;
    },
    planter(m){
      const g = new THREE.Group();
      const base = box(0.92, 0.42, 0.92, M.metalDark); base.position.y = 0.21; g.add(base);
      const hopper = box(0.5, 0.5, 0.5, M.metal); hopper.position.y = 0.55; g.add(hopper);
      const chute = box(0.16, 0.4, 0.16, M.metalDark); chute.position.set(0, 0.36, 0.42); g.add(chute);
      const rotor = new THREE.Group();
      for (let i = 0; i < 3; i++){
        const tine = box(0.04, 0.14, 0.02, M.metal);
        tine.position.y = 0.1; tine.rotation.x = i * Math.PI * 2 / 3;
        rotor.add(tine);
      }
      rotor.position.set(0, 0.22, 0.55); g.add(rotor);
      const lamp = box(0.12, 0.12, 0.12, M.glowOff); lamp.position.set(-0.32, 0.72, -0.32); g.add(lamp);
      m.animParts = { rotor, lamp };
      return g;
    },
    harvester(m){
      const g = new THREE.Group();
      const base = box(0.94, 0.5, 0.94, [M.metalDark, M.metalDark, M.vent, M.metalDark, M.metal, M.metalDark]);
      base.position.y = 0.25; g.add(base);
      const reel = new THREE.Group();
      for (let i = 0; i < 4; i++){
        const blade = box(0.05, 0.9, 0.05, M.metal);
        blade.position.x = 0.48; blade.rotation.z = i * Math.PI / 2;
        reel.add(blade);
      }
      reel.position.set(0.5, 0.45, 0); g.add(reel);
      const basket = box(0.4, 0.5, 0.8, M.chest); basket.position.set(-0.45, 0.5, 0); g.add(basket);
      const lamp = box(0.12, 0.12, 0.12, M.glowOff); lamp.position.set(0, 0.85, -0.3); g.add(lamp);
      m.animParts = { reel, lamp };
      return g;
    },
    collector(m){
      const g = new THREE.Group();
    ...
    vendor(m){
      const g = new THREE.Group();
      const body = box(0.96, 1.0, 0.96, [M.metal, M.metal, M.chestTop, M.metal, M.metal, M.metal]); body.position.y = 0.5; g.add(body);
      const screen = box(0.46, 0.32, 0.06, M.glow); screen.position.set(0, 0.66, 0.49); g.add(screen);
      const lamp = box(0.1, 0.1, 0.1, M.glowAmber); lamp.position.set(0.33, 1.26, 0); g.add(lamp);
      m.animParts = { body, lamp };
      return g;
    },
    trash(m){
      const g = new THREE.Group();
      const body = box(0.92, 1.0, 0.92, M.metalDark); body.position.y = 0.5; g.add(body);
      const mouth = box(0.64, 0.06, 0.64, M.dark); mouth.position.y = 1.02; g.add(mouth);
      m.animParts = { body };
      return g;
    },
    filter(m){
      const g = new THREE.Group();
      const body = box(0.9, 0.9, 0.9, [M.metal, M.metal, M.metal, M.metal, M.vent, M.metal]); body.position.y = 0.45; g.add(body);
      m.animParts = { body };
      return g;
    },
    chute(m){
      const g = new THREE.Group();
      const rim = box(0.92, 0.16, 0.92, M.metal); rim.position.y = 1.0; g.add(rim);
      const tube = box(0.6, 1.0, 0.6, M.metalDark); tube.position.y = 0.42; g.add(tube);
      m.animParts = { rim };
      return g;
    },
  };
  ...
  function machineData(type){
    switch (type){
      ...
      case 'planter':  return { seed: null };
      case 'harvester':return { out: null };
      case 'sellbot': return { list: null, buffer: null, earned: 0 };  // list=null 卖全部
      case 'vendor': return { out: null };
  ...
  function canMachineAccept(m, item){
    switch (m.type){
      ...
      case 'planter': {
        ...
        return !m.data.seed || (m.data.seed.item === item && m.data.seed.n < ITEMS[item].stack);
      }
      case 'sellbot': {
        if (!m.active || power.sat <= 0.05) return false;
        // 清单内物品到货即卖；非清单物品暂存缓冲格
        if (!m.data.list || m.data.list[item]) return true;
        return !m.data.buffer || (m.data.buffer.item === item && m.data.buffer.n < ITEMS[item].stack);
      }
      case 'vendor': return false;   // 出售站不从传送带收物
      case 'trash': return true;     // 销毁机来者不拒
      case 'filter':                 // 只放行清单内（list=null 全部放行）
        return !m.data.list || !!m.data.list[item];
      case 'chute': {
        const below = at(m.x, m.y - 1, m.z);
        if (below && below !== m && canMachineAccept(below, item)) return true;
        return !m.data.buffer || (m.data.buffer.item === item && m.data.buffer.n < ITEMS[item].stack);
      }
  ...
  function machineInsert(m, item){
    switch (m.type){
      ...
      case 'burner': case 'boiler':
        ...
        return true;
      case 'planter':
        if (!m.data.seed) m.data.seed = { item, n: 0 };
        m.data.seed.n++;
        return true;
      case 'trash': {   // 直接销毁
        m.data.destroyed++;
      ...
      case 'filter': {
        if (m.data.list && !m.data.list[item]) return false;
        if (!m.data.out) m.data.out = { item, n: 0 };
        if (m.data.out.item !== item) return false;
        m.data.out.n++;
        return true;
      }
      case 'chute': {   // 优先投入下方容器/机器，其次暂存
        const below = at(m.x, m.y - 1, m.z);
      ...
      case 'sellbot': {
        ...
        const onList = !m.data.list || m.data.list[item];
        if (onList){ sellToMarket(m, item); return true; }
        if (!m.data.buffer) m.data.buffer = { item, n: 0 };
        if (m.data.buffer.item !== item || m.data.buffer.n >= ITEMS[item].stack) return false;
        m.data.buffer.n++; return true;
      }
    }
    return false;
  }
  ...
  function irrigatorTick(m, dt){
    ...
    for (const c of cells){
      if (!c.c.water){
        c.c.water = true;
        World.markDirty(c.x, c.y, c.z);
      }
    }
  }
  function planterTick(m){
    ...
    if (Farm.plant(c.x, c.y, c.z, crop.id)){
      m.data.seed.n--;
      if (m.data.seed.n <= 0) m.data.seed = null;
      if (window.Sound) Sound.play('place', 0.6);
    }
  }
  function harvesterTick(m){
    ...
    for (const c of crops){
      ...
      for (const y of yields){
        if (!m.data.out.item) m.data.out = { item: y.item, n: 0 };
        if (m.data.out.item === y.item) m.data.out.n += y.n;
      }
    }
    while (m.data.out && m.data.out.n > 0 && tryOutput(m, m.data.out.item)){
      m.data.out.n--;
      if (m.data.out.n <= 0) m.data.out = null;
    }
  }

  // 随机矿物产出池：矿机不需压在矿石上，通电后按权重随机产出各类矿物
  const MINER_ORE_POOL = [
    { item: 'iron_ore', w: 34 },
    { item: 'copper_ore', w: 30 },
    { item: 'coal', w: 24 },
    { item: 'gold_ore', w: 6 },
    { item: 'titanium_ore', w: 4 },
    { item: 'uranium', w: 2 },
  ];
  function pickMinerOre(){
    let total = 0; for (const o of MINER_ORE_POOL) total += o.w;
    let r = Math.random() * total;
    for (const o of MINER_ORE_POOL){ r -= o.w; if (r <= 0) return o.item; }
    return 'iron_ore';
  }
  ...
  function tick(dt, dayFactor){
    ...
    for (const m of machines.values()) if (m.type === 'sellbot' || m.type === 'vendor' || m.type === 'irrigator' || m.type === 'planter' || m.type === 'harvester') m.active = power.sat > 0.05;
    ...
    for (const m of machines.values()){
      switch (m.type){
        ...
        case 'chest': case 'collector': collectorTick(m); break;   // 箱子也会自动输出到面前（皮带可取物）
        case 'vendor': {   // 售出货物自动输出到面前
          if (m.data.out && m.data.out.n > 0 && tryOutput(m, m.data.out.item)){
            m.data.out.n--;
            if (m.data.out.n <= 0) m.data.out = null;
          }
          break;
        }
        case 'filter': {   // 放行缓存自动输出
          if (m.data.out && m.data.out.n > 0 && tryOutput(m, m.data.out.item)){
            m.data.out.n--;
            if (m.data.out.n <= 0) m.data.out = null;
          }
        ...
        case 'chute': {   // 缓存向下投递
          if (m.data.buffer && m.data.buffer.n > 0){
            const below = at(m.x, m.y - 1, m.z);
            if (below && below !== m && canMachineAccept(below, m.data.buffer.item) && machineInsert(below, m.data.buffer.item)){
              m.data.buffer.n--;
              if (m.data.buffer.n <= 0) m.data.buffer = null;
            }
          }
          break;
        }
        case 'irrigator': irrigatorTick(m, dt); break;
        case 'planter': planterTick(m); break;
        case 'harvester': harvesterTick(m); break;
      }
    }
  }
    ...
    for (const m of machines.values()){
      ...
      switch (m.type){
        ...
        case 'irrigator':
          ...
          A.spray.scale.set(1 + Math.sin(animT * 8) * 0.15, 1 + Math.sin(animT * 7 + 1) * 0.12, 1);
          A.spray.material.opacity = m.active ? 0.18 + Math.sin(animT * 9) * 0.08 : 0;
          break;
        case 'planter':
          A.rotor.rotation.x += dt * (m.active ? 8 : 0.4);
          A.lamp.material = m.active ? M.glowGreen : (m.data.seed ? M.glowAmber : M.glowOff);
          break;
        case 'harvester':
          A.reel.rotation.z += dt * (m.active ? 6 : 0.3);
          A.lamp.material = m.active ? M.glowGreen : M.glowOff;
          break;
        case 'belt': {
