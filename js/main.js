...
const Game = (() => {
  ...
  const HOME_GALAXY_SEED = (typeof window !== 'undefined' && window.HOME_GALAXY_SEED) || 20240808;

  // ---------- 渲染器 ----------
  const renderer = new THREE.WebGLRenderer({ canvas: $('game'), antialias: true, powerPreference: 'high-performance' });
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
  renderer.setSize(window.innerWidth, window.innerHeight);
  const camera = new THREE.OrthographicCamera(-48, 48, 48, -48, -1000, 4000);
  if (World.setRayBack) World.setRayBack(-camera.near + 0.5);
  const camTarget = new THREE.Vector3(0, 40, 0);   // 相机注视的地面目标
  let camZoom = 26;                                // 可视高度（格）
  const CAM_TARGET_Y = 20;                         // 平面模式下相机高度恒定，避免地表起伏造成抽搐
  let camYaw = Math.PI / 4;                         // 默认 45° 斜对角视角（Q/E 旋转）
  const camVel = new THREE.Vector3();              // WASD 平滑速度
  let viewMode = 'iso';                            // iso | top | side
  let sideAxis = 'z', sideDir = 1, sideCut = 0;    // 泰拉剖面
  let fadeY = null;                                // Y 虚化阈值（null=关闭）
  let camTween = null;                             // 视图切换补间
  let tweenAnchorHit = null;
  const sidePlane = new THREE.Plane();
  const rimDir = new THREE.Vector3();
  const _crossH = new THREE.Vector3();
  ...
  function applyCamProjection(){
    const h = 64, w = h * (window.innerWidth / Math.max(1, window.innerHeight));
    camera.left = -w; camera.right = w;
    camera.top = h; camera.bottom = -h;
    camera.zoom = 64 / camZoom;
    camera.updateProjectionMatrix();
  }
  function updateSidePlane(){
    const n = sideAxis === 'z' ? new THREE.Vector3(0, 0, sideDir) : new THREE.Vector3(sideDir, 0, 0);
    sidePlane.set(n, -sideDir * sideCut);
    renderer.localClippingEnabled = true;
    renderer.clippingPlanes = [sidePlane];
  }
  function setViewModeState(){
    if (fadeY !== null){
      fadeY = null;
      World.setFadeAbove(null);
    }
    if (viewMode === 'side'){
      const axisKey = sideAxis === 'z' ? 'z' : 'x';
      sideCut = Math.floor(camTarget[axisKey]);
      camTarget[axisKey] = sideCut;
      updateSidePlane();
      World.setViewMask({ axis: sideAxis, dir: sideDir, cut: sideCut }, Math.floor(camTarget.x / 16), Math.floor(camTarget.z / 16));
      camera.up.set(0, 1, 0);
    } else {
      renderer.clippingPlanes = [];
      World.setViewMask(null);
      camera.up.set(0, 1, 0);
    }
  }
  function applyViewMode(){
    setViewModeState();
    UI.bigMessage('视图模式', viewMode === 'iso' ? '等距视角 · Q/E 旋转 · 方向键虚化' : viewMode === 'top' ? '俯视视角 · 方向键虚化' : '泰拉剖面 · 方向键切面/朝向', 1800);
  }
  function camPosForMode(anchor, mode){
    if (mode === 'top'){
      return new THREE.Vector3(anchor.x, anchor.y + camZoom, anchor.z);
    }
    if (mode === 'side'){
      const vd = sideAxis === 'z' ? new THREE.Vector3(0, 0, sideDir) : new THREE.Vector3(sideDir, 0, 0);
      return new THREE.Vector3(anchor.x, anchor.y, anchor.z).addScaledVector(vd, -camZoom);
    }
    const cp = Math.min(85, Math.max(10, settings.camPitch || 35)) * Math.PI / 180;
    const dir = new THREE.Vector3(Math.sin(camYaw) * Math.cos(cp), Math.sin(cp), Math.cos(camYaw) * Math.cos(cp));
    return new THREE.Vector3(anchor.x, anchor.y, anchor.z).addScaledVector(dir, camZoom);
  }
  function camUpForMode(mode){
    return mode === 'top' ? new THREE.Vector3(Math.sin(camYaw), 0, Math.cos(camYaw)) : new THREE.Vector3(0, 1, 0);
  }
  function startViewTween(nextMode){
    const hit = hoverHit;
    const anchor = new THREE.Vector3(
      hit ? hit.x + 0.5 : camTarget.x,
      CAM_TARGET_Y,
      hit ? hit.z + 0.5 : camTarget.z
    );
    tweenAnchorHit = hit;
    const fromP = camera.position.clone();
    const fromUp = camera.up.clone();
    viewMode = nextMode;
    camTarget.copy(anchor);
    setViewModeState();
    const toP = camPosForMode(anchor, viewMode);
    const toUp = camUpForMode(viewMode);
    camTween = { fromP, toP, fromUp, toUp, anchor, t: 0, dur: 0.55 };
    UI.bigMessage('视图模式', viewMode === 'iso' ? '等距视角 · Q/E 旋转 · 方向键虚化' : viewMode === 'top' ? '俯视视角 · 方向键虚化' : '泰拉剖面 · 方向键切面/朝向', 1800);
  }
  function updateCam(dt){
    if (camTween){
      ...
      camera.up.lerpVectors(camTween.fromUp, camTween.toUp, k).normalize();
      camera.lookAt(camTween.anchor);
      if (k >= 1){
        camTween = null;
        tweenAnchorHit = null;
        camera.up.copy(camUpForMode(viewMode)).normalize();
    ...
    const speed = Player.keys['ShiftLeft'] ? 60 : 26;
    const wish = new THREE.Vector3();
    if (viewMode === 'side'){
      if (Player.keys['KeyW']) wish.y += speed;
      if (Player.keys['KeyS']) wish.y -= speed;
      const look = sideAxis === 'z' ? new THREE.Vector3(0, 0, sideDir) : new THREE.Vector3(sideDir, 0, 0);
      const up = new THREE.Vector3(0, 1, 0);
      const right = new THREE.Vector3().crossVectors(look, up);
      if (Player.keys['KeyA']) wish.addScaledVector(right, -speed);
      if (Player.keys['KeyD']) wish.addScaledVector(right, speed);
    } else {
      const cosY = Math.cos(camYaw), sinY = Math.sin(camYaw);
      if (viewMode === 'top'){
        const up = new THREE.Vector3(sinY, 0, cosY);
        const right = new THREE.Vector3(-cosY, 0, sinY);
        if (Player.keys['KeyW']) wish.add(up);
        if (Player.keys['KeyS']) wish.sub(up);
        if (Player.keys['KeyA']) wish.addScaledVector(right, -speed);
        if (Player.keys['KeyD']) wish.addScaledVector(right, speed);
      } else {
        const f = new THREE.Vector3(-sinY, 0, -cosY);
        const r = new THREE.Vector3(cosY, 0, -sinY);
        if (Player.keys['KeyW']) wish.add(f);
        if (Player.keys['KeyS']) wish.sub(f);
        if (Player.keys['KeyA']) wish.sub(r);
        if (Player.keys['KeyD']) wish.add(r);
      }
    }
    if (wish.lengthSq() > 0) wish.normalize().multiplyScalar(speed);
    const k = 1 - Math.exp(-dt * 7);
    camVel.lerp(wish, k);
    camTarget.x += camVel.x * dt;
    camTarget.z += camVel.z * dt;
    camTarget.y += camVel.y * dt;
    if (viewMode === 'iso' || viewMode === 'top'){
      if (Player.keys['KeyQ'] && !UI.anyPanelOpen()) camYaw -= 1.6 * dt;
      if (Player.keys['KeyE'] && !UI.anyPanelOpen() && !dialogActive()) camYaw += 1.6 * dt;
    }
    camYaw = ((camYaw % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
    if (viewMode !== 'side') camTarget.y = CAM_TARGET_Y;
    if (viewMode === 'top'){
      camera.up.set(Math.sin(camYaw), 0, Math.cos(camYaw));
      camera.position.set(camTarget.x, camTarget.y + camZoom, camTarget.z);
      camera.lookAt(camTarget);
    } else if (viewMode === 'side'){
      camTarget[sideAxis] = sideCut;
      updateSidePlane();
      const vd = sideAxis === 'z' ? new THREE.Vector3(0, 0, sideDir) : new THREE.Vector3(sideDir, 0, 0);
      camera.up.set(0, 1, 0);
      camera.position.copy(camTarget).addScaledVector(vd, -camZoom);
      camera.lookAt(camTarget);
    } else {
      const cp = Math.min(85, Math.max(10, settings.camPitch || 35)) * Math.PI / 180;
      const dir = new THREE.Vector3(Math.sin(camYaw) * Math.cos(cp), Math.sin(cp), Math.cos(camYaw) * Math.cos(cp));
      camera.position.copy(camTarget).addScaledVector(dir, camZoom);
      camera.lookAt(camTarget);
    }
    rimDir.copy(camTarget).sub(camera.position).normalize();
    if (World.setRimDir) World.setRimDir(rimDir);
    syncCrossCam();
    // 泰拉/俯视下侧面大量正对相机，降低边缘高光避免满屏网格
    if (World.setRimStrength) World.setRimStrength(viewMode === 'iso' ? 0.5 : viewMode === 'top' ? 0.08 : 0);
    if (World.setDetailLOD) World.setDetailLOD(camZoom > 34 && viewMode !== 'side');
    applyCamProjection();
  }

  function syncCrossCam(){
    if (!World.setCrossCam) return;
    _crossH.copy(camTarget).sub(camera.position);
    _crossH.y = 0;
    if (_crossH.lengthSq() < 1e-8) _crossH.set(0, 0, 1);
    _crossH.normalize();
    World.setCrossCam(_crossH.x, _crossH.z, viewMode === 'iso' || viewMode === 'side' ? 1 : 0);
  }
  ...
  if (mobileDevice){
    ...
    settings.quality = 'high'; settings.planetLod = 'mid'; settings.clouds = 'on'; settings.realAtmo = 'on';
    camZoom = 8;
  }
  let lastQuality = null;
  function baseFov(){ return settings.fov; }
  function applySettings(){
    const mobileQ = settings.quality;
    World.setViewDist(mobileDevice ? Math.min(settings.chunkDist, 10) : (settings.chunkDist >= 33 ? 64 : settings.chunkDist));
    World.setFarDist(mobileDevice ? Math.min(settings.farDist, 768) : settings.farDist);
    // 画质：流畅=降采样渲染 · 标准=原生1x · 高画质=全分辨率+实时阳光阴影+电影级调色（MC光影风格）
    const q = mobileQ;
    // 手机保留标准材质、光照和特效，只降低实际渲染分辨率，避免高 DPR 放大 GPU 负载。
    ...
    if (q !== lastQuality){
      ...
      if (window.Space && Space.scene) Space.scene.traverse(recompile);
      lastQuality = q;
    }
    if (sunLight){
  ...
  let market = {};               // 物价波动
  let questIdx = 0;
  let farmStats = { till: 0, plant: 0, water: 0, harvest: 0 };
  ...
  function addPlanetSun(){
    let tex, sunPos, sunR;
    if (window.Space){
      tex = Space.sunTextures();
      sunPos = Space.SUN_POS;
      sunR = Space.SUN_R;
    } else {
      const c = document.createElement('canvas'); c.width = 64; c.height = 64;
      const x = c.getContext('2d');
      const g = x.createRadialGradient(32, 32, 2, 32, 32, 30);
      g.addColorStop(0, '#fff8d8'); g.addColorStop(0.4, '#ffd66e'); g.addColorStop(1, 'rgba(255,180,80,0)');
      x.fillStyle = g; x.fillRect(0, 0, 64, 64);
      const t = new THREE.CanvasTexture(c);
      t.magFilter = THREE.NearestFilter; t.minFilter = THREE.LinearFilter;
      tex = { surface: t, corona: t };
      sunPos = new THREE.Vector3(120, 180, 90);
      sunR = 40;
    }
    const pd = SYSTEM_PLANETS[currentPlanet];
    const dist = sunPos.distanceTo(new THREE.Vector3(...pd.pos));
    // 正交相机不做透视缩小，太阳/光晕必须用固定的小视尺寸，否则会糊满整个屏幕
    const appR = THREE.MathUtils.clamp(sunR / Math.max(1, dist) * 850, 2, 7);
    ...
    glow.scale.set(appR * 4.2, appR * 4.2, 1);
    glow.renderOrder = -3;
    ...
    corona.scale.set(appR * 2, appR * 2, 1);
  ...
  // ---------- 新游戏 / 换星球 ----------
  const loadFlavors = ['翻松土地', '播下种子', '培育作物', '修建风车', '唤醒机器之灵', '整理木箱'];
  async function genPlanet(pid, fresh, center){
    ...
    World.setCurve(0, 0, 0, 1, 1, 9999);
    if (!center) center = [0, 0];
    await World.pregen(center[0], center[1], 6, f => setP(f * 0.9));
    buildPlanetScene();
    if (saved && !fresh){
      Factory.deserialize(saved.machines);
      Farm.deserialize(saved.farm);
      if (saved.shipPos) camTarget.fromArray(saved.shipPos);
    } else {
      Farm.reset();
      const sp = World.findSpawn();
      camTarget.set(sp.x, sp.y - 2, sp.z);
    }
    registerSpawnFarm();
    camTarget.x = Math.round(camTarget.x);
    camTarget.z = Math.round(camTarget.z);
    shipHere = true;
    worldLoadedFor = pid;
    applyCamProjection();
    updateCam(0);
  ...
  function registerSpawnFarm(){
    ...
    if (!f || typeof Farm === 'undefined' || !Farm.register) return;
    for (let dz = 0; dz <= 16; dz++){
      for (let dx = 0; dx <= 10; dx++){
        if (dx === 4 || dx === 6) continue;   // 水渠不登记
        const wx = f.x + dx, wz = f.z + dz;
        const d = World.getDef(wx, f.h, wz);
        if (d && d.key === 'farmland') Farm.register(wx, f.h, wz);
      }
    }
  }
  function savePlanetState(){
    ...
    visitedPlanets[currentPlanet] = {
      ...
      farm: Farm.serialize(),
      shipPos: [camTarget.x, camTarget.y, camTarget.z],
      seed: w.seed,
  ...
  async function newGame(creativeMode, mult){
    ...
    if ($('quests')) $('quests').style.display = creative ? 'none' : '';
    UI.refreshAll();
    Sound.play('questNew');
    if (creative){
  ...
  async function startPastoral(){
    ...
    Player.inv.fill(null);
    Player.hotIdx = 0;
    Player.addItem('hoe', 1, true);
    Player.addItem('shovel', 1, true);
    Player.addItem('watering_can', 1, true);
    Player.addItem('wheat_seed', 12, true);
    Player.addItem('potato_seed', 12, true);
    Player.addItem('carrot_seed', 12, true);
    Player.addItem('beet_seed', 12, true);
    Player.addItem('pumpkin_seed', 12, true);
    Player.addItem('berry_seed', 12, true);
    Player.addItem('stone', 24, true);
    Player.addItem('planks_b', 24, true);
    $('boot').classList.add('hidden');
    UI.buildHotbar();
    await genPlanet(0, true, [0, 0]);
    $('hud').classList.remove('hidden');
    UI.refreshAll();
  ...
  function currentQuestChapter(){
    if (questIdx < 4) return '第一章 · 把荒地变成家园';
    if (questIdx < 8) return '第二章 · 让土地养活工业';
    if (questIdx < 11) return '第三章 · 风车与流水线';
    return questIdx < QUESTS.length ? '终章 · 自运转的田园' : '主线完成 · 星火长存';
  }
  function currentSeason(){ return ['春', '夏', '秋', '冬'][Math.floor((dayCount - 1) / 7) % 4]; }
  function profileStats(){
    return { playMin: Math.floor(playTime / 60), quest: `${Math.min(questIdx, QUESTS.length)}/${QUESTS.length}`, till: farmStats.till || 0, plant: farmStats.plant || 0, water: farmStats.water || 0, harvest: farmStats.harvest || 0, machines: window.Factory && Factory.machines ? Factory.machines.size : 0, tech: Object.keys(techState).filter(k => techState[k]).length, credits: Player.credits || 0 };
  }
  function questProgress(q){
    ...
    if (q.type === 'farm') return `${Math.min(farmStats[q.stat] || 0, q.n)}/${q.n}`;
    return null;
  }
  const placedCount = {};
  function checkQuest(){
    ...
    switch (q.type){
      ...
      case 'farm': done = (farmStats[q.stat] || 0) >= (q.n || 1); break;
    }
    if (done){
      questIdx++;
  ...
  function lockPointer(){
    // Pastoral Create 使用正交相机，不再需要指针锁定
  }
  ...
  // ---------- 输入（正交相机：WASD 平移 / 滚轮缩放 / 鼠标点击操作） ----------
  let _mx = 0, _my = 0;
  let hoverHit = null;           // 当前准星/鼠标悬停命中的方块，点击直接作用于它
  let mobileSelectedHit = null;  // 移动端点按锁定的目标，直到点选下一个方块
  const updatePointer = e => {
    // 移动端控件（摇杆/执行/热栏/菜单/视角钮）的触摸不改变鼠标落点，避免误选中/鬼魂跑到控件处
    if (mobileDevice && e.target && e.target.closest &&
        e.target.closest('#mobileControls, #mobileStick, #mbRightClick, .mobile-action, #mobileMenuDock, #hotbarWrap, #mbViewMode, #mobileMenuToggle')){
      return;
    }
  ...
  function mouseRay(){
    const ndc = new THREE.Vector2((_mx / Math.max(1, window.innerWidth)) * 2 - 1, -(_my / Math.max(1, window.innerHeight)) * 2 + 1);
    _raycaster.setFromCamera(ndc, camera);
    return _raycaster.ray;
  }
  function hitFromMouse(){
    const ray = mouseRay();
    return Player.lookTargetFromRay(ray.origin, ray.direction);
  }
  function updateHover(forceRay){
    const ray = mouseRay();
    if (state !== 'planet' || UI.anyPanelOpen()){
      hoverHit = null;
      Player.setHover(null);
      Player.updateGhostFromRay(ray.origin, ray.direction);
      return;
    }
    let hit;
    if (mobileDevice && !mobileSelectedHit && !forceRay){
      hit = null;   // 移动端未点选时不做射线悬停，避免误高亮/选中
    } else {
      hit = mobileSelectedHit && !forceRay ? mobileSelectedHit : Player.lookTargetFromRay(ray.origin, ray.direction);
    }
    hoverHit = hit;
    UI.showTraitBox(hit && hit.def && hit.def.key === 'crop' ? Farm.cropAt(hit.x, hit.y, hit.z) : null);
    Player.setHover(hit);
    // 移动端点选后：鬼魂预览在点选落点（否则会显示在手指/摇杆处）
    if (mobileSelectedHit && !forceRay && Player.updateGhostFromHit) Player.updateGhostFromHit(mobileSelectedHit);
    else Player.updateGhostFromRay(ray.origin, ray.direction);
  }
  function addLoot(yields){
    if (!yields) return;
    for (const y of yields){
      if (Player.addItem(y.item, y.n, true, y.tr) > 0 && UI.pickupToast) UI.pickupToast(y.item, y.n, y.tr);
    }
    UI.refreshAll();
  }
  function doLeftClick(){
    const hit = hoverHit;
    if (!hit) return;
    const sel = Player.hotIdx >= 0 ? Player.inv[Player.hotIdx] : null;
    const item = sel && ITEMS[sel.item];
    const def = hit.def;
    // 左键 = 使用工具：只有锄头能收割作物，只有铲子能挖掘方块
    if (!item){ Sound.play('uiError'); return; }
    if (item.id === 'hoe'){
      if (def.key === 'crop'){
        if (Farm.mature(hit.x, hit.y, hit.z)){
          addLoot(Farm.harvest(hit.x, hit.y, hit.z));
          Sound.play('craft');
        } else {
          const c = Farm.cropAt(hit.x, hit.y, hit.z);
          if (c){
            Player.addItem(c.def.seed, 1, true);
            c.cell.type = null; c.cell.stage = 0; c.cell.progress = 0; c.cell.tr = [];
    ...
    if (item.id === 'shovel'){
      if (def.key === 'crop'){ Sound.play('uiError'); return; }
      if (def.hard !== Infinity){
        Player.breakBlock(hit);
        UI.refreshAll();
      } else Sound.play('uiError');
      return;
    }
  ...
  function doRightClick(){
    ...
    const sel = Player.hotIdx >= 0 ? Player.inv[Player.hotIdx] : null;
    const item = sel && ITEMS[sel.item];
    // 铲子右键：优先破坏目标方块（含机器），而不是“使用”
    if (item && item.id === 'shovel'){
      const def = hit.def;
      if (def && def.hard !== Infinity){
        Player.breakBlock(hit);
        UI.refreshAll();
      } else Sound.play('uiError');
      return;
    ...
    if (m && !item){ UI.openMachinePanel(m); return; }   // 只有空手（未持物品）才能交互机器
    if (!item){ Sound.play('uiError'); return; }
    if (item.id === 'hoe'){
      if (Farm.hoe(hit.x, hit.y, hit.z)){ Sound.play('place'); UI.refreshAll(); }
      else Sound.play('uiError');
      return;
    }
    if (item.id === 'watering_can'){
      if (Farm.water(hit.x, hit.y, hit.z)){ Sound.play('insert'); UI.refreshAll(); }
      else Sound.play('uiError');
      return;
    }
    const crop = Object.values(CROPS).find(c => c.seed === item.id);
    if (crop){
      const cell = hit.def.key === 'farmland' ? { x: hit.x, y: hit.y, z: hit.z } : null;
      if (cell && Farm.plant(cell.x, cell.y, cell.z, crop.id, sel.tr)){
        if (!Player.removeVariant(item.id, sel.tr || [])) Player.removeItem(item.id, 1);
        Sound.play('place');
        UI.refreshAll();
      } else Sound.play('uiError');
      return;
    ...
    if (item.block){
      if (mobileSelectedHit && Player.tryPlaceAtHit) Player.tryPlaceAtHit(mobileSelectedHit);
      else Player.tryPlaceFromRay(ray.origin, ray.direction);
      return;
    }
    Sound.play('uiError');
  }
  function doMobileSmartAction(){
    if (state !== 'planet' || UI.anyPanelOpen()) return;
    if (!mobileSelectedHit){ Sound.play('uiError'); UI.setInteractHint('先点击田地、作物或方块进行选中'); return; }
    hoverHit = mobileSelectedHit;
    const selected = Player.hotIdx >= 0 ? Player.inv[Player.hotIdx] : null;
    const item = selected && ITEMS[selected.item];
    const machine = Factory.at(hoverHit.x, hoverHit.y, hoverHit.z);
    if (machine || !item || item.id === 'watering_can' || item.block || Object.values(CROPS).some(c => c.seed === item.id)) doRightClick();
    else if (item.id === 'hoe'){
      if (hoverHit.def.key === 'crop') doLeftClick(); else doRightClick();
    } else if (item.id === 'shovel') doLeftClick();
    else doRightClick();
  }
  window.addEventListener('pointerdown', e => {
    ...
    else if (e.button === 2) doRightClick();
  }, true);
  window.addEventListener('contextmenu', e => e.preventDefault(), true);
  window.addEventListener('wheel', e => {
    if (state !== 'planet' || UI.anyPanelOpen()) return;
    const min = viewMode === 'side' ? 10 : 8, max = viewMode === 'side' ? 80 : 64;
    camZoom = THREE.MathUtils.clamp(camZoom + (e.deltaY > 0 ? 3 : -3), min, max);
    updateCam(0);
    e.preventDefault();
  }, { passive: false, capture: true });

  // ---------- 触屏（手机横屏）：左摇杆移动 / 点按选中 / 智能执行 ----------
  let mobileActionMode = 'use';            // 'use' = 左键工具 | 'place' = 右键放置/播种
  const touchPts = new Map();              // pointerId -> {x, y}
  let touchPan = false, pinchStart = 0, tapStart = 0, tapMoved = 0, tapId = null;
  let tapHandled = false, mobileActionHeld = false, mobileActionUsed = false;
  function zoomForMode(){ return viewMode === 'side' ? [10, 80] : [8, 64]; }
  function setMobileActionMode(mode){
    mobileActionMode = mode;
    const b = $('mbAction');
    if (b) b.textContent = mode === 'place' ? '▸ 放置' : '▸ 使用';
  }
  window.addEventListener('pointerdown', e => {
    if (e.target !== $('game')) return;
    if (state !== 'planet' || UI.anyPanelOpen()) return;
    ...
    if (touchPts.size === 1){
      touchPan = true;
      tapId = e.pointerId; tapStart = performance.now(); tapMoved = 0;
  ...
  window.addEventListener('pointermove', e => {
    ...
    if (state !== 'planet' || UI.anyPanelOpen()) return;
    if (touchPts.size === 1 && touchPan){
      tapMoved += Math.abs(dx) + Math.abs(dy);
      const scale = camZoom / Math.max(1, window.innerHeight);
      let fx, fy, fz, rx, ry, rz;
      if (viewMode === 'top'){
        fx = Math.sin(camYaw); fy = 0; fz = Math.cos(camYaw);
        rx = Math.cos(camYaw); ry = 0; rz = -Math.sin(camYaw);
      } else if (viewMode === 'side'){
        fx = sideAxis === 'z' ? 0 : sideDir;
        fy = 0; fz = sideAxis === 'z' ? sideDir : 0;
        rx = sideAxis === 'z' ? 1 : 0; ry = 0; rz = 0;
      } else {
        const cp = Math.min(85, Math.max(10, settings.camPitch || 35)) * Math.PI / 180;
        fx = -Math.sin(camYaw) * Math.cos(cp); fy = -Math.sin(cp); fz = -Math.cos(camYaw) * Math.cos(cp);
        rx = Math.cos(camYaw); ry = 0; rz = -Math.sin(camYaw);
      }
      // 屏幕 y 方向在 iso/top 下对应水平前进方向，side 下对应竖直
      const fwdDy = viewMode === 'side' ? -fy * dy : -(fx * dy);
      const fwdDz = viewMode === 'side' ? -fz * dy : -(fz * dy);
      camTarget.x += (fwdDy - rx * dx) * scale;
      camTarget.y += (viewMode === 'side' ? -fy * dy : 0) * scale;
      camTarget.z += (fwdDz - rz * dx) * scale;
      updateCam(0);
    } else if (touchPts.size === 2 && pinchStart > 0){
      const a = [...touchPts.values()];
      const d = Math.hypot(a[0].x - a[1].x, a[0].y - a[1].y);
      const [min, max] = zoomForMode();
      camZoom = THREE.MathUtils.clamp(camZoom * (pinchStart / Math.max(1, d)), min, max);
      pinchStart = d;
      updateCam(0);
    }
  }, true);
  const finishTouch = e => {
    ...
    if (touchPts.size < 2) pinchStart = 0;
    if (tapId === e.pointerId && touchPts.size === 0){
      const isTap = tapMoved < 14 && (performance.now() - tapStart) < 400;
      tapId = null;
      if (isTap && state === 'planet' && !UI.anyPanelOpen() && e.target === $('game')){
        _mx = e.clientX; _my = e.clientY;
        updateHover(true);
        mobileSelectedHit = hoverHit;
        tapHandled = true;
        if (mobileActionHeld){
          mobileActionUsed = true;
          doMobileSmartAction();
        } else {
          UI.setInteractHint('<b>已选中</b> 点击右侧「执行」进行操作');
        }
      }
    }
    if (touchPts.size === 0) touchPan = false;
  };
  window.addEventListener('pointerup', finishTouch, true);
  window.addEventListener('pointercancel', finishTouch, true);
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code === 'KeyM' && state === 'planet'){ togglePlanetMap(); }
    if (e.code === 'KeyB' && state === 'planet'){ UI.openTrade(); }
    if (e.code === 'KeyV' && state === 'planet' && !UI.anyPanelOpen()){
      viewMode = viewMode === 'iso' ? 'top' : viewMode === 'top' ? 'side' : 'iso';
      applyViewMode();
      updateCam(0);
    }
    if (e.code === 'ArrowUp' || e.code === 'ArrowDown' || e.code === 'ArrowLeft' || e.code === 'ArrowRight'){
      ...
      if (viewMode === 'side'){
        if (e.code === 'ArrowUp' || e.code === 'ArrowDown'){
          sideCut += (e.code === 'ArrowUp' ? sideDir : -sideDir);
          sideCut = Math.max(-1024, Math.min(1024, sideCut));
          camTarget[sideAxis] = sideCut;
          World.setViewMask({ axis: sideAxis, dir: sideDir, cut: sideCut }, Math.floor(camTarget.x / 16), Math.floor(camTarget.z / 16));
          updateCam(0);
        } else {
          const order = ['z+', 'x+', 'z-', 'x-'];
          let i = order.indexOf(sideAxis + (sideDir > 0 ? '+' : '-'));
          i = (i + (e.code === 'ArrowRight' ? 1 : -1) + 4) % 4;
          sideAxis = order[i][0]; sideDir = order[i][1] === '+' ? 1 : -1;
          sideCut = Math.floor(camTarget[sideAxis]);
          camTarget[sideAxis] = sideCut;
          applyViewMode();
          updateCam(0);
        }
      } else if (e.code === 'ArrowUp' || e.code === 'ArrowDown'){
        const cur = fadeY === null ? 24 : fadeY;
        fadeY = THREE.MathUtils.clamp(cur + (e.code === 'ArrowUp' ? 1 : -1), 4, 63);
        if (fadeY >= 63) fadeY = null;
        World.setFadeAbove(fadeY);
        UI.bigMessage('虚化高度', fadeY === null ? '已关闭' : 'Y ' + fadeY + ' 以上虚化', 900);
      }
    }
    if (e.code === 'Escape'){
      if (e.ctrlKey || e.code === 'F8'){ toggleErrPanel(); return; }
      if (dialogActive()){ closeDialog(); return; }
      if (UI.anyPanelOpen()){ UI.closeAll(); Sound.play('uiClose'); }
      else UI.toggle('pausePanel');
    }
    if (e.code === 'F8'){ toggleErrPanel(); return; }
    if (e.code === 'F5'){ e.preventDefault(); if (save()) UI.bigMessage('已快速存档', '', 1200); }
    if (e.code === 'KeyR' && state === 'planet'){ UI.openEncyclopediaFromHover(); }
    if (e.code === 'KeyU' && state === 'planet'){ UI.openEncyclopediaFromHover(true); }
    if (e.code.startsWith('Digit')){
      const n = +e.code.slice(5);
      if (state === 'planet'){
        // 1 号位 = 交互位（🖐，空手交互方块）；2-9 号位 = 物品
        if (n === 0 || n === 1){ Player.hotIdx = -1; UI.refreshHotbar(); UI.showItemName(); }
        else if (n >= 2 && n <= 9){ Player.hotIdx = n - 2; UI.refreshHotbar(); UI.showItemName(); }
      }
    }
    if (Player.keys['KeyX'] && state === 'planet' && !UI.anyPanelOpen()){
      // 按住 X + WASD：调整放置朝向（W北 S南 A西 D东），不移动相机
      const dirMap = { KeyD: 0, KeyS: 1, KeyA: 2, KeyW: 3 };
      if (dirMap[e.code] !== undefined){
        e.preventDefault();
        Player.setPlaceDir(dirMap[e.code]);
        UI.bigMessage('放置朝向', ['东 +X','南 +Z','西 -X','北 -Z'][dirMap[e.code]], 900);
        Player.keys[e.code] = false;   // 不触发相机平移
        return;
      }
    }
    if (e.code === 'KeyE' && !e.repeat && dialogActive()) advanceDialog();
  });
  document.addEventListener('keyup', e => { Player.keys[e.code] = false; });

  // 移动端视角切换按钮（等距 / 俯视 / 泰拉剖面）
  const mbVM = document.getElementById('mbViewMode');
  if (mbVM){
    const viewLabels = { iso: '等距', top: '俯视', side: '剖面' };
    const syncVM = () => { mbVM.textContent = viewLabels[viewMode] || '视角'; };
    mbVM.onclick = () => {
      if (state !== 'planet' || UI.anyPanelOpen()) return;
      viewMode = viewMode === 'iso' ? 'top' : viewMode === 'top' ? 'side' : 'iso';
      applyViewMode();
  ...
  function localDayTime(){
    ...
    const sp = window.Space ? Space.planets.find(p => p.def.id === currentPlanet) : null;
  ...
  function computeEntry(planet){
    ...
    return { target: null, x: e.x, z: e.z };
  }
  ...
  function updateSurfacePreview(planet, bestD){
    ...
    World.setCurve(0, v.x, v.z, 1, 1, 9999);
  }
  ...
  // ---------- 空间站（重制版）：未加载 station.js 时以空桩兼容，避免旧胶水代码报错 ----------
  const Station = window.Station || { onDocked: null, onBuyShip: null, onGarage: null, closeDialog(){}, tryBegin(){ return false; }, update(){ return null; }, walking: false, phase: '' };
  let __stFrames = 0;
  if ($('btnUndock')) $('btnUndock').style.display = 'none';   // 旧「离站」按钮弃用：离站=停机位按 W
  Station.onDocked = () => {
    flags.docked = true;
  ...
  Station.onBuyShip = (v) => {
    ...
    if (Player.credits < u.price){
      Sound.play('uiError');
      UI.bigMessage('金币不足', `需要 🪙${u.price.toLocaleString()}，当前 🪙${Player.credits.toLocaleString()}`, 2600);
      return false;
    }
  ...
  // 一次性清除旧版 STARFORGE 存档（与田园新档不兼容，避免混入）
  try {
    for (const k of Object.keys(localStorage)) if (k.indexOf('starforge') === 0) localStorage.removeItem(k);
  } catch(e){}

  function listSaves(){
    // 旧版 STARFORGE 单存档不再迁移：避免旧档混入田园世界
  ...
  function buildSaveData(){
    ...
    return {
      ...
      shipState: state !== 'planet' && window.Space ? {
  ...
  function saveTo(key, name){
    ...
    const idx = listSaves();
    if (!key){
      key = 'pastoral_sv_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
    ...
    try {
      ...
      writeIndex(idx);
      activeSaveKey = key;
      if (window.Cloud && Cloud.logged){
        const cloudData = buildSaveData();
        Cloud.upload(key, cloudData, { name: entry.name, planetName: entry.planetName, playMin: entry.playMin })
          .catch(e => { if (e.status === 409 && Cloud.resolveConflict) Cloud.resolveConflict(key, cloudData, { name: entry.name, planetName: entry.planetName, playMin: entry.playMin }, e); else console.warn('[cloud save]', e.message); });
      }
      return true;
  ...
  async function loadFrom(key){
    ...
    if (d.v && d.v < 3){
      UI.bigMessage('旧版 STARFORGE 档案不兼容', 'Pastoral Create 使用全新存档，请从主菜单开始新档');
      return false;
    }
    if (d.v < 4){
      // v3 -> v4：重置田园任务进度，清理太空专属物品
      d.questIdx = 0; d.flags = {}; d.farmStats = { till: 0, plant: 0, water: 0, harvest: 0 };
      if (d.player && Array.isArray(d.player.inv)){
        const drop = new Set(['fuel', 'tritium', 'antimatter', 'warpcell', 'launchpad_b', 'beacon_b']);
        d.player.inv = d.player.inv.map(s => s && drop.has(s.item) ? null : s);
      }
    ...
    if (window.Space) Space.restoreGalaxy(d.galaxySeed !== undefined ? d.galaxySeed : HOME_GALAXY_SEED);
    currentPlanet = d.currentPlanet; dayTime = d.dayTime; dayCount = d.dayCount || 1; playTime = d.playTime || 0;
    questIdx = d.questIdx; flags = d.flags; techState = d.techState; market = d.market;
    farmStats = Object.assign({ till: 0, plant: 0, water: 0, harvest: 0 }, d.farmStats || {});
    ...
    if (d.state === 'space' && d.shipState && window.Space){
      savePlanetState();
      Space.enter(currentPlanet);
  ...
  async function loadCloudData(data){
    ...
    const key = 'pastoral_cloud_import_' + Date.now();
  ...
  function updateDayNight(dt){
    const before = dayTime;
    dayTime = (dayTime + dt / DAY_LEN) % 1;
    if (dayTime < before){
      dayCount++;
      const season = currentSeason();
      Farm.newDay(season);
      UI.bigMessage(`第 ${dayCount} 天 · ${season}`, '水润且当季的作物在夜里长大了', 1800);
    }
    // 太阳方向：优先按真实太空几何投影到本地切平面（与太空中的恒星方位一致，换系无跳变）
    const curSp = window.Space ? Space.planets.find(p => p.def.id === currentPlanet) : null;
    let sunH;
    if (curSp){
    ...
    if (sunLight){
      ...
      for (const sp of skyPlanets){
        const other = curSp && window.Space ? Space.planets.find(p => p.def.id === sp.userData.pid) : null;
  ...
  function refreshEnv(dt){
    ...
    if (info){
      info.innerHTML = `坐标 <span>${Math.floor(camTarget.x)}, ${Math.floor(camTarget.z)}</span> · ${hour.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}`;
    }
    if ($('hazIcon')) $('hazIcon').textContent = '';
    UI.refreshHUD();
  }
  ...
  function refreshHints(){
    ...
    if (state === 'planet'){
      const hit = hitFromMouse();
      if (hit && Factory.at(hit.x, hit.y, hit.z)){
        const mm = Factory.at(hit.x, hit.y, hit.z);
        UI.setInteractHint(`<b>右键</b> 打开 ${BLOCK_BY_ID[World.get(hit.x, hit.y, hit.z)].name}`);
        return;
      }
      if (hit && hit.def.key === 'crop' && Farm.mature(hit.x, hit.y, hit.z)){
        UI.setInteractHint('<b>左键+锄头</b> 收割成熟作物');
        return;
      }
      if (hit && hit.def.key === 'crop'){
        UI.setInteractHint('<b>左键+锄头</b> 清除未成熟作物');
        return;
      }
      if (hit && hit.def.key === 'farmland'){
        UI.setInteractHint('<b>右键</b> 播种 / 浇水 / 锄地');
        return;
      }
      const sel = Player.hotIdx >= 0 ? Player.inv[Player.hotIdx] : null;
      const item = sel && ITEMS[sel.item];
      if (item && item.id === 'shovel') UI.setInteractHint('<b>左键</b> 挖掘方块');
      else if (item && item.block) UI.setInteractHint('<b>右键</b> 放置 ' + ITEMS[item.id].name);
  ...
  function flatWorldToCanvas(wx, wz){
    ...
    return { x: (wx - m.cx) * m.scale + W / 2, y: (wz - m.cz) * m.scale + H / 2 };
  }
  function initFlatMap(){
    ...
    canvas.addEventListener('wheel', e => {
      ...
      const before = flatCanvasToWorld(mx, my);
      const k = e.deltaY < 0 ? 1.18 : 1 / 1.18;
      mapFlat.scale = THREE.MathUtils.clamp(mapFlat.scale * k, 0.35, 3.2);
      const after = flatCanvasToWorld(mx, my);
      mapFlat.cx += before.x - after.x;
      mapFlat.cz += before.z - after.z;
      drawFlatMap();
    }, { passive: false });
  }
  function flatMapClick(e){
    const rect = $('mapCanvas').getBoundingClientRect();
    const w = flatCanvasToWorld(e.clientX - rect.left, e.clientY - rect.top);
    mapPending = { x: Math.round(w.x), z: Math.round(w.z) };
    $('mapAddForm').classList.remove('hidden');
    $('mapAddPos').textContent = `选中坐标 X ${mapPending.x} · Z ${mapPending.z}`;
    $('mapMarkName').value = '';
    $('mapMarkName').focus();
    Sound.play('uiClick');
    drawFlatMap();
  ...
  function buildFlatTerrain(){
    ...
    const key = Math.round(m.cx * m.scale) + ',' + Math.round(m.cz * m.scale) + ',' + m.scale.toFixed(2);
    if (m.texKey === key) return;
    m.texKey = key;
    const img = m.ctx.createImageData(W, H);
    const d = img.data;
    const step = 2;
    for (let py = 0; py < H; py += step){
      const wz = m.cz + (py - H / 2) / m.scale;
      for (let px = 0; px < W; px += step){
        const wx = m.cx + (px - W / 2) / m.scale;
        const c = World.mapColorRGB(wx, wz);
    ...
    // 区块网格
    m.ctx.strokeStyle = 'rgba(255,255,255,0.10)';
    m.ctx.lineWidth = 1;
    const halfW = W / 2 / m.scale, halfH = H / 2 / m.scale;
    for (let x = Math.floor((m.cx - halfW) / 16) * 16; x <= m.cx + halfW; x += 16){
      const p = flatWorldToCanvas(x, m.cz);
      m.ctx.beginPath(); m.ctx.moveTo(p.x, 0); m.ctx.lineTo(p.x, H); m.ctx.stroke();
    }
    for (let z = Math.floor((m.cz - halfH) / 16) * 16; z <= m.cz + halfH; z += 16){
      const p = flatWorldToCanvas(m.cx, z);
      m.ctx.beginPath(); m.ctx.moveTo(0, p.y); m.ctx.lineTo(W, p.y); m.ctx.stroke();
    }
  }
  function drawMarker(wx, wz, color, label, shape){
    const p = flatWorldToCanvas(wx, wz);
    const canvas = $('mapCanvas');
    if (p.x < -16 || p.x > canvas.width + 16 || p.y < -16 || p.y > canvas.height + 16) return;
    const ctx = mapFlat.ctx;
    ctx.save();
    ...
    if (shape === 'square'){
      ...
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.moveTo(0, -5); ctx.lineTo(4.5, 4); ctx.lineTo(-4.5, 4);
    ...
    if (label){
      ...
      ctx.lineWidth = 2;
      ctx.strokeText(label, 7, 4); ctx.fillText(label, 7, 4);
    }
    ctx.restore();
  }
  function drawFlatMap(){
    if (!mapFlat || !World.biome) return;
    if (mapFlat.follow){
      mapFlat.cx = camTarget.x;
      mapFlat.cz = camTarget.z;
    }
    buildFlatTerrain();
    if (window.Factory){
      for (const k of Factory.machines.keys()){
        const p = k.split(',').map(Number);
        drawMarker(p[0], p[2], '#35e0e8', '', 'square');
      }
    }
    for (const st of (World.structures || [])){
      drawMarker(st.x, st.z, st.type === 'village' ? '#4dc86a' : '#d8b038', st.name || '', 'square');
    }
    for (const mk of (mapMarks[currentPlanet] || [])) drawMarker(mk.x, mk.z, '#ffd94d', mk.label || '标记', 'diamond');
    if (mapPending) drawMarker(mapPending.x, mapPending.z, '#ff4444', '✚', 'diamond');
    drawMarker(camTarget.x, camTarget.z, '#7dff8a', '我', 'diamond');
    $('mapInfo').textContent = `X ${Math.floor(camTarget.x)} · Z ${Math.floor(camTarget.z)} · 标记 ${(mapMarks[currentPlanet] || []).length} 个`;
  }
  ...
  function tickMapPanel(){
    if (!$('mapPanel').classList.contains('hidden')) drawFlatMap();
  }
  ...
  // 构建水印：右下角常驻小字（station 态升级为实时仪表：阶段/相机/朝向逐帧显示）
  {
    ...
    bd.textContent = 'Pastoral Create · dev';
    bd.style.cssText = 'position:fixed;right:6px;bottom:4px;font-size:11px;color:rgba(160,210,230,0.85);z-index:9999;pointer-events:none;font-family:monospace;text-shadow:0 1px 2px #000;display:none';
    try { if (new URLSearchParams(location.search).has('debug') || localStorage.getItem('st_debug') === '1') bd.style.display = ''; } catch(e){}
  ...
  function loop(){
    ...
    if (state === 'planet'){
      ...
      Player.setToolVisible(false);
      Player.updateDrops(dt);
      Player.tickParticles(dt);
      updateCam(dt);
      if (!mobileDevice || !mobileSelectedHit) updateHover();
      World.stream(camTarget.x, camTarget.z);
      World.update(dt, camTarget.x, camTarget.z);
      ...
      refreshEnv(dt);
      refreshHints();
      updateGroundClouds(dt, camTarget.x, camTarget.z);
      tickDialog(dt);
  ...
  function updateMarkers(dt){
    ...
    // 地图标记：常驻定位标签
    updateMapMarkers(inPanel, refPos);
    // 联机队友标记
  ...
  let mapMarkEls = [];
  const _mapMarkPos = new THREE.Vector3();
  function updateMapMarkers(inPanel, refPos){
    const marks = (mapMarks[currentPlanet] || []).map(m => ({ x: m.x, y: m.y || 30, z: m.z, label: (m.gal ? '✦' : '') + (m.label || '标记') }));
    while (mapMarkEls.length < marks.length) mapMarkEls.push(makeMarkEl('map', '#ffd94d', '⚑'));
    while (mapMarkEls.length > marks.length) mapMarkEls.pop().remove();
    for (let i = 0; i < marks.length; i++){
      const b = marks[i], el = mapMarkEls[i];
      if (inPanel){ el.style.display = 'none'; continue; }
      _mapMarkPos.set(b.x + 0.5, b.y + 2.5, b.z + 0.5);
      const dist = refPos.distanceTo(_mapMarkPos);
      if (dist < 6){ el.style.display = 'none'; continue; }
      const s = worldToScreen(_mapMarkPos, true);
      if (!s){ el.style.display = 'none'; continue; }
      el.style.display = '';
      el.classList.remove('edge');
      el.style.left = Math.max(30, Math.min(window.innerWidth - 30, s.x)) + 'px';
      ...
      if (s.behind || s.x < 30 || s.x > window.innerWidth - 30 || s.y < 40 || s.y > window.innerHeight - 40)
        el.classList.add('edge');
      el.querySelector('.wm-tx').textContent = `${b.label} ${dist.toFixed(0)}m`;
    }
  }
  ...
  function updateSpaceMarkers(){
    ...
    for (const mk of marks){
      if (mk.poi){
        ...
        if (p){
          const lon = mk.poi.x * 0.004 + p.mesh.rotation.y;
          const lat = THREE.MathUtils.clamp(mk.poi.z * 0.004, -1.15, 1.15);
          _bDir.set(Math.cos(lat) * Math.cos(lon), Math.sin(lat), Math.cos(lat) * Math.sin(lon));
  ...
  // ---------- 菜单绑定 ----------
  const tips = [
    '「锄头翻开泥土，种子落进耕地，日子就有了盼头。」',
    '「洒水壶只浇一格，灌溉机能照顾 5×5 的田野。」',
    '「收割机把成熟作物送上传送带，烘焙麦香就交给熔炉。」',
    '「收购站静静立在传送带末端，按行情收购每一样抵达的物产并当场付现。」',
    '「放置方向由你点击的方块面决定，不用再手动转向。」',
    '「采矿机放在矿脉正上方，再给它接上电。R 键能查配方。」',
  ];
  let tipIdx = 0;
  setInterval(() => {
    if (state !== 'menu') return;
  ...
  // ---------- 移动端按钮栏 ----------
  if ($('mbRightClick')){
    $('mbInventory').onclick = () => { Sound.play('uiClick'); UI.toggle('invPanel'); };
    const closeMobileMenu = () => { $('mobileMenuPanel').classList.add('hidden'); $('mobileMenuToggle').classList.remove('open'); $('mobileMenuToggle').setAttribute('aria-expanded', 'false'); };
    let mobileMenuClickAt = 0;
    const toggleMobileMenu = e => {
      if (e) e.preventDefault();
      // Touch browsers may synthesize a duplicate click after pointer input; ignore that second event.
      const now = performance.now(); if (now - mobileMenuClickAt < 250) return; mobileMenuClickAt = now;
      const panel = $('mobileMenuPanel'); const opened = panel.classList.toggle('hidden') === false;
    ...
    menuAction('mnuTech', () => UI.toggle('techPanel'));
    menuAction('mnuBreed', () => UI.openBreeding());
    ...
    actionButton.onpointerup = e => { if (!mobileActionUsed) doMobileSmartAction(); mobileActionHeld = false; e.preventDefault(); };
    actionButton.onpointercancel = () => { mobileActionHeld = false; };
    const finePointer = window.matchMedia && window.matchMedia('(pointer: fine)').matches;
    $('mobileControls').classList.toggle('hidden', finePointer);
    const stick = $('mobileStick'), knob = $('mobileStickKnob');
    let stickId = null;
    const releaseStick = () => {
      stickId = null;
      ['KeyW','KeyA','KeyS','KeyD'].forEach(k => Player.keys[k] = false);
      knob.style.transform = '';
    };
    stick.onpointerdown = e => { stickId = e.pointerId; stick.setPointerCapture(e.pointerId); e.preventDefault(); };
    stick.onpointermove = e => {
      if (e.pointerId !== stickId || state !== 'planet' || UI.anyPanelOpen()) return;
      const r = stick.getBoundingClientRect(), max = r.width * 0.28;
      let x = e.clientX - (r.left + r.width / 2), y = e.clientY - (r.top + r.height / 2);
      const len = Math.hypot(x, y); if (len > max){ x *= max / len; y *= max / len; }
      knob.style.transform = `translate(${x}px,${y}px)`;
      const dead = max * 0.28;
      // 手持可放置方块时：摇杆调整放置朝向，而非移动
      const handSel = Player.hotIdx >= 0 ? Player.inv[Player.hotIdx] : null;
      const placing = !!(handSel && ITEMS[handSel.item] && ITEMS[handSel.item].block);
      if (placing && (Math.abs(x) > dead || Math.abs(y) > dead)){
        const dir = Math.abs(x) > Math.abs(y) ? (x > 0 ? 0 : 2) : (y > 0 ? 1 : 3);
        if (Player.setPlaceDir) Player.setPlaceDir(dir);
        // 已点选落点时：刷新鬼魂预览，让方向变化立即可见
        if (mobileSelectedHit && Player.updateGhostFromHit) Player.updateGhostFromHit(mobileSelectedHit);
        UI.setInteractHint(`放置朝向：${['东 +X','南 +Z','西 -X','北 -Z'][dir]}`);
        e.preventDefault();
        return;
      }
      Player.keys['KeyW'] = y < -dead; Player.keys['KeyS'] = y > dead;
      Player.keys['KeyA'] = x < -dead; Player.keys['KeyD'] = x > dead;
      e.preventDefault();
    };
  ...
  function refreshSettingsUI(){
    ...
    document.querySelectorAll('#setItemPack button').forEach(b =>
      b.classList.toggle('on', b.dataset.q === Tex.savedItemPackName()));
  }
  ...
  document.querySelectorAll('#setCamPitch button').forEach(b => {
    b.onclick = () => {
      Sound.play('uiClick');
      settings.camPitch = +b.dataset.q;
      try { localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings)); } catch(e){}
      refreshSettingsUI();
    };
  });
  document.querySelectorAll('#setTexPack button').forEach(b => {
    b.onclick = () => { Sound.play('uiClick'); Tex.setPack(b.dataset.q); refreshSettingsUI(); };
  });
  document.querySelectorAll('#setItemPack button').forEach(b => {
  ...
  $('importPackZip').onchange = async e => {
    ...
    if (!f) return;
    try {
      const entries = await Tex.readZip(f);
      await Tex.importFiles(entries);
      UI.bigMessage('材质包已导入', `匹配 ${entries.filter(x => /\.png$/i.test(x.path)).length} 张图片`, 3600);
    } catch(err){
      UI.bigMessage('ZIP 读取失败', String(err && err.message || err), 4200);
    }
    e.target.value = '';
  };
  $('importPackDir').onchange = async e => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;
    try {
      const entries = files.map(f => ({ path: f.webkitRelativePath || f.name, blob: f }));
      await Tex.importFiles(entries);
      UI.bigMessage('材质包已导入', `匹配 ${entries.filter(x => /\.png$/i.test(x.path)).length} 张图片`, 3600);
    } catch(err){
      UI.bigMessage('文件夹导入失败', String(err && err.message || err), 4200);
    }
    e.target.value = '';
  };
  ...
  if ($('btnNet')) $('btnNet').onclick = () => { Sound.play('uiOpen'); openNetPanel(); };
  if ($('btnNetBoot')) $('btnNetBoot').onclick = () => { Sound.begin(); Sound.play('uiOpen'); openNetPanel(); };
  if (window.Net) Net.statusChanged = () => { $('netStatus').textContent = Net.status(); };
  if ($('btnNetHost')) $('btnNetHost').onclick = async () => {
    Sound.play('uiClick');
  ...
  if ($('btnNetJoin')) $('btnNetJoin').onclick = async () => {
    Sound.play('uiClick');
    $('netStatus').textContent = '连接中…';
    try {
  ...
  if ($('btnNetLeave')) $('btnNetLeave').onclick = () => {
    Sound.play('uiClose');
    Net.disconnect();
    $('netStatus').textContent = Net.status();
  };
  ...
  // 自动存档会顺带上传云端：统一入口避免本地与云端两个定时器并发覆盖同一档位。
  setInterval(() => { if (state === 'planet' && !paused && activeSaveKey) save(); }, 90000);
  ...
  const api = {
    ...
    get mobileActionMode(){ return mobileActionMode; },
    directLoot: true,
    joinGame,
    ...
    techDone, completeTech, currentQuests, currentQuestId, onBlockMined, onBlockPlaced, onFarmEvent, lockPointer,
    setWarpLock,
...
// 默认材质包：PureEdge 方块 + Whimscape 物品 + Pastoral 作物。
// 必须等 const Game 初始化完成后再加载，否则 refreshQuests 读取 Game 时会抛 ReferenceError 并中断贴图覆盖。
try {
  localStorage.setItem('texture_pack', 'default');
