...
const Player = (() => {
  ...
  // 背包：36 格（前8=快捷栏）；hotIdx=-1 表示选中固定栏位「采矿激光」
  ...
  let hiliteFace = null, hiliteFaceEdge = null;
  ...
  function sameTraits(a, b){
    const item = a && a.item;
    const traitStack = item && ITEMS[item] && (ITEMS[item].iconFn === 'seed' || Object.values(CROPS).some(c => c.produce === item));
    if (traitStack) return true;
    const ta = a && a.tr, tb = b && b.tr;
    if (!ta || !ta.length) return !tb || !tb.length;
    if (!tb || ta.length !== tb.length) return false;
    return ta.every(id => tb.includes(id));
  }
  function addItem(item, n = 1, silent, traits){
    let left = n;
    const max = ITEMS[item].stack;
    const tr = traits && traits.length ? traits.slice() : null;
    for (const s of inv){
      if (s && s.item === item && sameTraits(s, { item, tr }) && s.n < max){
        const traitStack = ITEMS[item].iconFn === 'seed' || Object.values(CROPS).some(c => c.produce === item);
        if (traitStack && tr && tr.length){
          s.variants = s.variants || (s.tr && s.tr.length ? [{ tr: s.tr.slice(), n: s.n }] : []);
          const variant = s.variants.find(v => v.tr.length === tr.length && v.tr.every(id => tr.includes(id)));
          if (variant) variant.n += Math.min(left, max - s.n);
          else s.variants.push({ tr: tr.slice(), n: Math.min(left, max - s.n) });
          if (!s.tr || !s.tr.length) s.tr = tr.slice();
        }
    ...
    if (left) for (let i = 0; i < inv.length; i++){
      if (!inv[i]){
        ...
        inv[i] = { item, n: add, ...(tr ? { tr, variants: [{ tr: tr.slice(), n: add }] } : {}) }; left -= add;
        if (!left) break;
      }
    }
    const added = n - left;
  ...
  // 统计指定词条组合在该物品堆叠中的总数量
  function countVariant(item, traits){
    let n = 0;
    for (const s of inv){
      if (!s || s.item !== item) continue;
      const variants = s.variants && s.variants.length ? s.variants : [{ tr: s.tr || [], n: s.n }];
      const v = variants.find(x => x.tr.length === traits.length && x.tr.every(id => traits.includes(id)));
      if (v) n += v.n;
    }
    return n;
  }
  function removeVariant(item, traits){
    for (let i = inv.length - 1; i >= 0; i--){
      const s = inv[i];
      if (!s || s.item !== item) continue;
      const variants = s.variants || (s.tr && s.tr.length ? [{ tr: s.tr, n: s.n }] : []);
      const v = variants.find(x => x.tr.length === traits.length && x.tr.every(id => traits.includes(id)) && x.n > 0);
      if (!v) continue;
      v.n--; s.n--;
      if (s.variants){
        s.variants = variants.filter(x => x.n > 0);
        s.tr = s.variants[0] ? s.variants[0].tr.slice() : [];
      }
      if (s.n <= 0) inv[i] = null;
      UI.refreshAll();
      return true;
  ...
  // 从混合堆叠中拆出指定词条组合的全部数量，作为独立堆叠返回；找不到返回 null
  function splitVariant(item, traits){
    for (let i = inv.length - 1; i >= 0; i--){
      const s = inv[i];
      if (!s || s.item !== item) continue;
      const variants = s.variants || (s.tr && s.tr.length ? [{ tr: s.tr, n: s.n }] : []);
      const v = variants.find(x => x.n > 0 && x.tr.length === traits.length && x.tr.every(id => traits.includes(id)));
      if (!v) continue;
      const n = v.n;
      v.n = 0; s.n -= n;
      if (s.variants){
      ...
      if (s.n <= 0) inv[i] = null;
      UI.refreshAll();
      return { item, n, tr: traits.slice(), variants: [{ tr: traits.slice(), n }] };
    }
    return null;
  }
  ...
  function initVisuals(scene){
    ...
    hiliteMesh = new THREE.LineSegments(edges, new THREE.LineBasicMaterial({ color: 0xfff2b0, transparent: true, opacity: 0.95 }));
    hiliteMesh.name = 'hiliteBox';
    hiliteMesh.visible = false;
    scene.add(hiliteMesh);
    hiliteFace = new THREE.Mesh(
      new THREE.PlaneGeometry(1.01, 1.01),
      new THREE.MeshBasicMaterial({ color: 0xffc040, transparent: true, opacity: 0.62, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide })
    );
    hiliteFace.name = 'hiliteFace';
    hiliteFace.visible = false;
    scene.add(hiliteFace);
    hiliteFaceEdge = new THREE.LineSegments(
      new THREE.EdgesGeometry(new THREE.PlaneGeometry(1.005, 1.005)),
      new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 1 })
    );
    hiliteFaceEdge.name = 'hiliteFaceEdge';
    hiliteFaceEdge.visible = false;
    scene.add(hiliteFaceEdge);
    // 挖掘裂纹（黑色渐显盒）
  ...
  function dirFromFace(face){
    if (face[0] === 1) return 0;    // 东 +X
    if (face[2] === 1) return 1;    // 南 +Z
    if (face[0] === -1) return 2;   // 西 -X
    if (face[2] === -1) return 3;   // 北 -Z
    return effectiveDir();          // 顶/底面没有水平方向，沿用视线朝向
  }
  function cycleRot(){
  ...
  function setPlaceDir(dir){ placeDirOverride = (dir === null || dir === undefined) ? null : ((dir % 4) + 4) % 4; }   // 摇杆/外部设置放置朝向（null=自动）
  function placeTarget(camera){
    const dir = new THREE.Vector3();
    camera.getWorldDirection(dir);
    return placeTargetFromRay(camera.position.clone(), dir);
  }
  function placeTargetFromRay(origin, dir){
    const sel = inv[hotIdx];
    const item = sel && ITEMS[sel.item];
    if (!item || !item.block) return null;
    const hit = World.raycast(origin.clone(), dir, 320);
    if (!hit) return null;
    const px = hit.x + hit.face[0], py = hit.y + hit.face[1], pz = hit.z + hit.face[2];
    if (!World.inBounds(px, py, pz)) return null;
    // 剖面视图：目标格子若在相机侧（被裁剪隐藏），夹到最近的可见格
    if (World.viewMask){
      const m = World.viewMask;
      const v = m.axis === 'x' ? px : pz;
      const nv = m.dir > 0 ? Math.max(v, m.cut) : Math.min(v, m.cut);
      if (m.axis === 'x') px = nv; else pz = nv;
    }
    let ok = World.getDef(px, py, pz).id === 0;
    // 手动设置了放置朝向（R 键 / 摇杆 / X+WASD）时优先，否则按命中面朝向
    return { px, py, pz, ok, item, faceDir: placeDirOverride !== null ? effectiveDir() : dirFromFace(hit.face) };
  }
  function updateGhostPreview(camera){
    const dir = new THREE.Vector3();
    camera.getWorldDirection(dir);
    updateGhostFromRay(camera.position.clone(), dir);
  }
  function renderGhost(t){
    if (!t || !t.item || !BLOCKS[t.item.block]){
      ghostGroup.visible = false;
      return;
    }
    ...
    if (bDef.machine){
      ...
      const d = t.faceDir !== undefined ? t.faceDir : effectiveDir();
  ...
  function updateGhostFromRay(origin, dir){
    if (!ghostGroup) return;
    if (Game.state !== 'planet' || UI.anyPanelOpen()){ ghostGroup.visible = false; return; }
    renderGhost(placeTargetFromRay(origin, dir));
  }
  // 移动端：鬼魂预览显示在“点选方块”的落点
  function updateGhostFromHit(hit){
    if (!ghostGroup) return;
    if (Game.state !== 'planet' || UI.anyPanelOpen()){ ghostGroup.visible = false; return; }
    const sel = inv[hotIdx];
    const item = sel && ITEMS[sel.item];
    if (!hit || !item || !item.block){ ghostGroup.visible = false; return; }
    const px = hit.x + hit.face[0], py = hit.y + hit.face[1], pz = hit.z + hit.face[2];
    if (!World.inBounds(px, py, pz)){ ghostGroup.visible = false; return; }
    const ok = World.getDef(px, py, pz).id === 0;
    renderGhost({ px, py, pz, ok, item, faceDir: placeDirOverride !== null ? effectiveDir() : dirFromFace(hit.face) });
  }
  ...
  function breakBlock(hit){
    ...
    const give = (item, n) => {
      if (n <= 0) return;
      if (window.Game && Game.directLoot) Player.addItem(item, n, true);
      else spawnDrop(cx, cy, cz, item, n);
    };
    if (def.machine){
      ...
      if (m){
        const d = m.data;
        const giveStack = (s) => { if (s && s.n > 0) give(s.item, s.n); };
        giveStack(d.in); giveStack(d.fuel); giveStack(d.out);
        if (d.slots) d.slots.forEach(giveStack);
        if (typeof d.cargo === 'number' && d.cargo > 0) give('carbon', d.cargo);
        if (d.in && typeof d.in === 'object' && !d.in.item)
          for (const k in d.in) if (typeof d.in[k] === 'number' && d.in[k] > 0) give(k, d.in[k]);
        if (d.items) d.items.forEach(it => give(it.item, 1));
      }
    } else {
      ...
      if (above.cross){
        dropsOf(above).forEach(d => give(d.item, d.n * Game.dropMult));
        World.set(hit.x, hit.y + 1, hit.z, 0);
      }
    }
    dropsOf(def).forEach(d => give(d.item, d.n * Game.dropMult));
  ...
  function tryPlace(camera){
    const dir = new THREE.Vector3();
    camera.getWorldDirection(dir);
    tryPlaceFromRay(camera.position.clone(), dir);
  }
  function doPlaceAt(px, py, pz, blockKey, faceDir){
    const bDef = BLOCKS[blockKey];
    if (bDef.machine){
      Factory.place(px, py, pz, blockKey, faceDir !== undefined ? faceDir : effectiveDir());
      Sound.play('machinePlace');
      Game.onBlockPlaced(blockKey);
    } else {
  ...
  function tryPlaceFromRay(origin, dir){
    const sel = inv[hotIdx];
    if (!sel || !ITEMS[sel.item] || !ITEMS[sel.item].block) return;
    const t = placeTargetFromRay(origin, dir);
    if (!t || !t.ok){ if (t && !t.ok) Sound.play('uiError'); return; }
    doPlaceAt(t.px, t.py, t.pz, t.item.block, t.faceDir);
  }
  // 移动端：按“点选的方块”放置，落点由点选方块的面决定（而非手指/摇杆处的射线）
  function tryPlaceAtHit(hit){
    if (!hit) return;
    const sel = inv[hotIdx];
    const item = sel && ITEMS[sel.item];
    if (!item || !item.block) return;
    const px = hit.x + hit.face[0], py = hit.y + hit.face[1], pz = hit.z + hit.face[2];
    if (!World.inBounds(px, py, pz)){ Sound.play('uiError'); return; }
    if (World.getDef(px, py, pz).id !== 0){ Sound.play('uiError'); return; }
    doPlaceAt(px, py, pz, item.block, placeDirOverride !== null ? effectiveDir() : dirFromFace(hit.face));
  }
  ...
  function lookTargetFromRay(origin, dir){
    return World.raycast(origin.clone(), dir, 320);
  }
  const _faceN = new THREE.Vector3();
  const _faceUp = new THREE.Vector3(0, 0, 1);
  const _faceQ = new THREE.Quaternion();
  function setHover(pos){
    if (!hiliteMesh) return;
    if (!pos){
      hiliteMesh.visible = false;
      if (hiliteFace) hiliteFace.visible = false;
      if (hiliteFaceEdge) hiliteFaceEdge.visible = false;
      return;
    }
    hiliteMesh.visible = true;
    hiliteMesh.position.set(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5);
    hiliteMesh.material.opacity = 0.9 + Math.sin(performance.now() * 0.008) * 0.1;
    if (hiliteFace && pos.face){
      _faceN.set(pos.face[0] || 0, pos.face[1] || 0, pos.face[2] || 0);
      if (_faceN.lengthSq() === 0) _faceN.set(0, 1, 0);
      _faceN.normalize();
      _faceQ.setFromUnitVectors(_faceUp, _faceN);
      hiliteFace.position.set(
        pos.x + 0.5 + _faceN.x * 0.505,
        pos.y + 0.5 + _faceN.y * 0.505,
        pos.z + 0.5 + _faceN.z * 0.505
      );
      hiliteFace.quaternion.copy(_faceQ);
      hiliteFace.material.opacity = 0.58 + Math.sin(performance.now() * 0.008) * 0.12;
      hiliteFace.visible = true;
      hiliteFaceEdge.position.copy(hiliteFace.position);
      hiliteFaceEdge.quaternion.copy(_faceQ);
      hiliteFaceEdge.material.opacity = 0.95 + Math.sin(performance.now() * 0.008) * 0.05;
      hiliteFaceEdge.visible = true;
    } else {
      if (hiliteFace) hiliteFace.visible = false;
      if (hiliteFaceEdge) hiliteFaceEdge.visible = false;
    }
  ...
  function serialize(){
    return { pos: pos.toArray(), yaw, pitch, stats: { ...stats }, inv: inv.map(s => s ? { ...s, variants: s.variants && s.variants.map(v => ({ tr: v.tr.slice(), n: v.n })) } : null), hotIdx, credits };
  }
  function deserialize(d){
    pos.fromArray(d.pos); yaw = d.yaw; pitch = d.pitch;
    Object.assign(stats, d.stats);
    for (let i = 0; i < inv.length; i++) inv[i] = d.inv[i] ? { ...d.inv[i], variants: d.inv[i].variants && d.inv[i].variants.map(v => ({ tr: v.tr.slice(), n: v.n })) } : null;
    hotIdx = d.hotIdx || 0;
    credits = d.credits || 0;
  }
  ...
  return {
    ...
    keys, update, updateDrops, initVisuals, tryPlace, tryPlaceFromRay, tryPlaceAtHit, lookTarget, lookTargetFromRay, setHover, updateGhostFromHit,
    updateGhostFromRay, placeTargetFromRay, breakBlock, recharge, damage, setToolVisible,
    chargeStat, canCharge, CHARGE_DEFS, cycleRot, setPlaceDir,
    addItem, removeItem, removeVariant, splitVariant, countVariant, countItem, hasItems, payItems, throwHeld, spawnDrop,
