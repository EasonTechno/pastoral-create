...
const Space = (() => {
  ...
  function destroyVisitor(v){
    ...
    if (window.UI && UI.refreshHUD) UI.refreshHUD();   // 金币即时刷新（不等背包交互）
    if (window.UI) UI.bigMessage(`☠ 击毁 ${u.cls} 级「${SHIP_MODEL_NAMES[u.model] || u.model}」`, `战利品入舱：${txt.join(' · ')} · 金币 +${loot.cr.toLocaleString()}`, 4200);
    removeVisitorShip(v);
